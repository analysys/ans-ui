...
export default {
  ...
  methods: {
    ...
    setSelectionRange (selectionStart, selectionEnd) {
      ...
      const input = this.$refs.timepickerPoptipInput.querySelector('input')
    ...
    blur () {
      ...
      setTimeout(() => {
        const input = this.$refs.timepickerPoptipInput.querySelector('input').value

        if (this.readonly || this.disabled || !input || this.pattern !== 'edit') return

        if (this.type === 'time') {
          const date = moment(this.dateValue || new Date()).format('YYYY-MM-DD') + ' ' + input
          if (isDate(date)) {
          ...
          const setDate = (index, input) => {
            return moment(this.dateValue[index] || new Date()).format('YYYY-MM-DD') + ' ' + input
          }
          const inputRange = input.split(' - '); const inputDate = setDate(0, inputRange[0]); const inputDate1 = setDate(1, inputRange[1])
