  ...
  <div class="x-date-packer-ym">
    ...
    <div class="x-date-packer-item">
      <div class="x-date-packer-year" v-if="type === 'y'">
        <div class="year-item" v-for="item in yearList" :key="item"><span :class="{act: isYear(item)}" @click="selectYear(item)">{{item}}</span></div>
      </div>
      <div class="x-date-packer-month" v-else>
        <div class="year-item" v-for="item in 12" :key="item"><span :class="{act: isMonth(item)}" @click="selectMonth(item)">{{t('ans.datepicker.month' + item)}}</span></div>
      </div>
    </div>
  </div>
</template>
