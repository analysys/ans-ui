  ...
  <div class="x-date-packer-panel" ref="dateDay">
    ...
    <div class="x-date-packer-day">
      <div class="x-date-packer-day-week">
        <span v-for="(item, i) in week" :key="i" class="lattice">{{item}}</span>
      </div>
...
export default {
  ...
  methods: {
    init (date) {
      const { year, month, week, day } = this.thisDate()
      this.year = year
      this.month = month
      this.weeks = week
      this.days = day
    ...
    // 获取当前面板信息
    thisDate () {
      const dates = moment(new Date(this.toDate)).toDate(); const year = dates.getFullYear(); const month = dates.getMonth() + 1
      return {
        year: year, // 年份
        month: month, // 月份
    ...
    //
    hoverDate (d) {
      const toDate = moment(new Date(this.year, this.month - 1, d)).format('YYYYMMDD')
      const hStart = this.hoverStartDate; const hEnd = this.hoverEndDate
      ...
      if (selDate.length > 0) {
        ...
        for (let i = 0, len = selDate.length; i < len; i++) {
          if (isValid(selDate[i])) {
            const thisDate = moment(selDate[i]).format('YYYYMMDD')
  ...
  directives: {
    attr: {
      bind: (el, binding) => {
        const colors = colorAlls
        const { toDate, hStart, hEnd } = binding.value
        if (hStart && hEnd) {
          for (let i = 0, len = hStart.length; i < len; i++) {
            if (hStart[i] && hEnd[i]) {
              const startDate = moment(new Date(hStart[i])).format('YYYYMMDD')
              const endDate = moment(new Date(hEnd[i])).format('YYYYMMDD')
